...
- **A standalone department is judged on nothing but its own criteria, and stays out of the whole-school figures.** `Standalone` on the `Faculties` tab marks it; groups under it inherit that. Tutor time is the case it was built for: what a good tutor session looks like has little in common with what a good lesson looks like, so the whole-school criteria would answer the wrong question, and folding it into the teaching figures would move them for reasons that are not about teaching. It still appears on the Master tab, in a section of its own below the whole-school figures, and behaves like any other department when opened by name.
...
- `REPARENT` and `RETIRED_CRITERIA` in `AddCriteria.gs` carry out decisions the ordinary runs will not make on their own: moving a department that is already placed, and removing a criterion filed against a department it should never have been on. Each entry is a decision already taken, and one already carried out does nothing.
...
- The observer types the part **before** the @; the school's domain is fixed beside the field and added server-side. A whole address is accepted only at that domain, so a record of somebody's teaching cannot leave the school through a typo. The name is prefilled from the `Staff` tab's `Email` column where it has one, and shown for checking either way, because a guessed address is how a record reaches the wrong person. Adding an `Email` column to `Staff` fills the suggestion in; without it the observer types the name.
...
- Each criterion is **Met, Not met or N/A**. N/A is for one that does not apply to this set of books: it is recorded, and left out of the met rate rather than counting against the teacher.
- A scrutiny of one pupil's books can name them and carry a disadvantaged band of 1 to 4, both optional, stored in `Student` and `Disadvantaged`. The field asks for initials: the sheet is not the place for a named record of a child.
...
- The class list comes from the warehouse in one call (`distinct: "class_code"`) and is cached for six hours, so no manual list is needed. Faculty is inferred from the code via the `Arbor_Subjects` tab (Subject abbreviation or name, Faculty), which `setSubjectMap()` fills from `SUBJECT_MAP`. Both spellings are listed, since class codes use both (`10Y/En2` gives `En`, `10/English` gives `English`). The optional `Classes` tab pins a faculty or a teacher to a code where the inference is wrong. Single class codes can still be typed straight into the per-class sampler on the scrutiny form.
